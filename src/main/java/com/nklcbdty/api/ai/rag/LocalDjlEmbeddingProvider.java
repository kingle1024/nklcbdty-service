...
/**
 * 로컬 한국어/영어 다국어 문장 임베딩 (DJL + PyTorch + HuggingFace).
 *
 ...
 *
 * <p>모델 + PyTorch 네이티브 런타임은 수백 MB 의 메모리·디스크를 쓴다. 작은 컨테이너
 * (예: 0.5GB)에서는 기동 자체가 실패하므로, 여유 있는 호스트에서만
 * {@code nklcb.rag.embedding.provider=local} 로 켠다. 기본 공급자가
 * {@link OpenAiEmbeddingProvider} 인 이유다.</p>
 *
...
@Slf4j
@Component
@ConditionalOnProperty(name = "nklcb.rag.embedding.provider", havingValue = "local")
public class LocalDjlEmbeddingProvider implements EmbeddingProvider {

    private static final String MODEL_URL =
            "djl://ai.djl.huggingface.pytorch/sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2";
    private static final String MODEL_VERSION = "paraphrase-multilingual-MiniLM-L12-v2";
    ...
    public void load() {
        try {
            ...
            log.info("임베딩 provider=local 모델 로드 완료: {}", MODEL_VERSION);
        } catch (Throwable t) {
