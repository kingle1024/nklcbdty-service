...
public class CrawlerCommonService {
    ...
    public PersonalHistoryDto extractPersonalHistoryFromJobPage(String url) {
        try {
            // 예전엔 coupang.jobs 만 브라우저 세션 쿠키를 하드코딩해 붙였지만, 그 쿠키는 이미 만료됐고
            // 쿠팡은 Greenhouse API 로 옮겨 이 경로를 타지 않는다. 남은 호출자(네이버/라인)는
            // 공통 커넥션(브라우저 User-Agent)으로 통일한다.
            Document doc = jsoupConnect(url).timeout(3000).get();
